...
A: 尝试将您操作系统的 DNS 服务器地址修改为公共 DNS，例如 \`8.8.8.8\` 或 \`1.1.1.1\`。`,
  ...
  // 🍏 B.2.6 macOS 专项 (macOS Specifics)
  {
    keywordGroups: [
      // 场景一：直接描述错误信息
      ['(mac|macos)', '(提示|报错)', '已损坏', '(无法打开|打不开)'],
      ['(mac|macos)', '(无法|不能)验证开发者|来自身份不明的开发者|未识别的开发者'],
      ['(mac|macos)', '将对您的电脑造成伤害|恶意软件'],
      ['(mac|macos)', '(移到|扔到|丢到)废纸篓'],

      // 场景二：通用描述无法打开的问题
      ['(mac|macos|苹果电脑)', '(软件|客户端|程序|app|应用)', '(打不开|无法打开|启动不了|无法启动|没反应|无响应|闪退|点不开|运行不了|无法运行)'],
      ['(mac|macos)', '(下载|安装)了?', '(打不开|用不了|没反应)'],

      // 这个模式不要求必须有“软件”等主语
      ['(mac|macos)(系统|系统的)?', '(怎么|咋|为啥|就是)?(打不开|无法打开|启动不了|用不了|没反应)', '(求助|指导|怎么办|哪位|大(侠|佬|哥)?)?'],

      // 场景三：询问解决方案中的关键词
      ['(mac|macos)', '任何来源', '(没有|找不到|怎么开|如何启用)'],
      ['(mac|macos)', '(xattr|quarantine|隔离)', '(移除|删除|命令)'],
      ['(mac|macos)', '(怎么|如何)', '(签名|codesign)'],

      // 英文场景
      ['(mac|macos)', '(app|application|program)', '(can.?t|won.?t) open|damaged|not working|crash(es)?'],
      ['(mac|macos)', 'developer cannot be verified|unidentified developer'],
      ['(mac|macos)', 'is damaged and can.?t be opened'],
      ['(mac|macos)', 'move to (trash|bin)'],
    ],
...
\`\`\`
同样地，在命令末尾（\`-\` 后面）**加一个空格**，然后将应用图标拖入终端窗口来填充路径。如果提示需要安装命令行工具，请同意安装后再执行此命令。`,
  },
  ...
  },
  {
    ...
    keywordGroups: [
      ...
      [
        ...
        '(突然|就)?(不能?(启动|运行|上网|打开|用|访问|连接)|(启动|运行|用|上|连|访问)不(了|动|起来)|打不开(网站|网页)?|没(有)?网(络)?|挂了|崩了|没反应|失败|出问题|报错|寄了|坏了)',
      ],
      ...
      ['(昨天|之前|本来|原来)还?(好好的|能用|正常)', '(今天|现在|突然|怎么)就?(不行了|用不了|坏了)'],
      ['(啥也没干|没动过)', '就?(用不了|坏了|不行了)'],

      // 模式四：“为啥/怎么”疑问句式
      ['(为啥|怎么|咋)', '(突然|一下|忽然)?(就)?(不能?用(了)?|用不(了|动)|坏了|不行了|没反应了|连不上|没网了|无法连接)'],
