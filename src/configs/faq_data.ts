...
3. **管理员权限 (Windows)**：前往 **设置 -> 通用**，启用 **以管理员身份运行** 并重启客户端。`,
  },
  {
    keywordGroups: [
      ...
      ['开机|自启(动)?', '(托盘|图标)', '(变)?透明|消失|(空)?白|不见了|没了|(点)?不动|没反应|([不无没]法|不能)(点击|操作)'],
      // 描述另一种常见情况：任务管理器里有进程，但托盘图标有问题
      ['(在|有)?.*?进程.*?(在|有)?', '托盘?图标', '(变)?透明|消失|不见了|点?不动'],
      // 英文匹配
      ['(on|after) (boot|startup|start)', 'tray icon', 'transparent|disappear|gone|missing|unclickable|no response|doesn.?t work|is invisible'],
    ],
...
3.  建议将延迟时间设置为 **10 秒或更长**。如果问题依然存在，可以尝试继续增加延迟时间（例如 15 或 20 秒），直到图标能够稳定显示。`,
  },
  {
    keywordGroups: [
      ...
      // 英文模式
      ['GUI|client|program|exe|installer', 'antivirus|defender|firewall|security', 'delete|quarantine|block|remove|flagged as|report as'],
...
**重要提示**: 添加排除后，如果文件已被隔离或删除，你需要**重新解压或安装客户端**到该受信任的文件夹中，即可正常使用。`,
  ...
  {
    keywordGroups: [
      ['滚动(发行)?', '更新|升级|update|upgrade', '[没无]法|不能|失败|不动|卡住|报错|出问题|无效|没反应|没用|(还是|仍然|依然)老版'],
      // --- 新增英文关键词 ---
...
\`\`\``,
  },
  {
    keywordGroups: [
      ...
      ['(怎么|如何|咋|怎样)', '(快速|快捷|手动)?(导入|添加|加入|粘贴|使用)', '(单个|单独)?(的)?节点|分享链接|(ss|vmess|trojan)链接'],
      // 模式二：特定协议的链接怎么用
...
3. 定期在 **插件中心** 更新 \`滚动发行\` 插件。`,
  },
  {
    ...
    keywordGroups: [
      // 模式一：【操作后】-> [突发性负面结果]
      [
        '(?:今天|刚刚)?(?:更新|升级|重启|安装|设置).*?(?:后|完|好)?',
        '(?:突然|就)?(?:(?:不能|[无没]法)(?:启动|运行|上网|打开|用|访问|连接)|(?:启动|运行|用|上网|连接|访问)不(?:了|动|起来|上)|打不开(?:网站|网页)?|没(?:有)?网(?:络)?|闪退|死机|卡(?:住|了)|(?:挂|崩|坏|寄)了|没反应|失败|出问题|报错|(还|仍|依).*?老)',
      ],

      // 模式二：【主体] -> [负面结果]
      ['内核|程序|软件|客户端', '([不没无]法?|不能|没(有)?).*(启动|运行)|(启动|运行).*(不了|不起来|失败|没反应)|启动失败'],
      ...
      // --- 新增模式六：【代理副作用】开启后影响局部网络 ---
      // 规则 6.1: 句式 -> (开启代理后)[定语](对象)(发生故障)
      [
        '(开(了|启)|用(了)?|一开)(代理|客户端|软件|程序)',
        '(国内|有些|部分|有的|特定)?(网站|网页|图片|APP|应用|游戏)',
        '(就)?(打不开|加载不(了|出来)|访问不了|访问异常|显示不全|特别慢|转圈|没反应|连不上|进不去|图片错误)',
      ],

      // 规则 6.1.1: 句式 -> (关掉代理后)(能访问)
      ['(代理|软件|内核|tun|gui|sing|clash|mihomo)', '(关|停|禁|结束)', '能.*?(访问|打开|连接|查看|用)'],

      // 规则 6.2: 句式 -> (对象)(发生故障)，(关闭代理后)(恢复正常)  <- 精准匹配你的范例
      [
        '(国内|有些|部分|有的)?(网站|网页|图片|APP|应用|游戏)',
        '(打不开|加载不(了|出来)|访问不了|访问异常|显示不全|特别慢|转圈|没反应|连不上|进不去)',
        '(代理|客户端|软件)?(一关|关(了|掉)|退出)(就|马上)?(好|正常|恢复)(了)?',
      ],
      // 规则 6.3: 句式 -> 更口语化的直接抱怨
      ['(代理|客户端|软件)', '(影响|好像影响|会导致|让我|搞得我)', '(国内|有些|部分)?(网站|网页|APP|应用|网速)', '(打不开|变慢|访问不了|加载不出来)'],
      ...
      // 新增英文模式: 匹配代理副作用
      ['(when|after) (enabling|turning on) the (proxy|client)', '(some|certain|domestic) (websites|images|apps) (fail to load|won.?t open|get slow)'],
      ['(some|certain|domestic) (websites|images) (load fine|work again) after i (disable|turn off) the (proxy|client)'],
    ],
    excludeKeywords: [
...
请严格按照以下顺序，确保你的软件环境是最新且配置正确的：

1.  **更新主程序**: 前往 **软件设置 -> 关于** 页面，检测并更新 GUI 客户端至最新版本。
2.  **启用滚动发行**: 前往 **软件设置 -> 通用** 页面，确保 **启用滚动发行** 选项已启用。
3.  **清理旧版本文件**: 删除 **程序目录** 下的 **data/rolling-release** 文件夹。
4.  **更新滚动发行**: 前往 **插件中心**，安装或更新 **滚动发行** 插件至最新版本，并运行。
5.  **更新内核**: 前往 **软件设置 -> 内核** 页面，检测并更新内核至最新版本。
...
如果你的问题与 **TUN 模式** 相关（例如，开启后无法上网），请仔细遵循以下步骤：
...
如果你已尝试上述所有方案但问题依旧，为了得到有效的帮助，请尽可能详细地提供以下信息。同时，也请一并说明**其余任何你认为可能相关的补充情况**（例如：是否使用了其他网络软件、系统是否有特殊的安全设置、问题是否只在特定网络环境下出现等），以便我们能够全面地分析和定位问题：

**1. 环境与上下文信息**
*   **操作系统**: (例如: Windows 11 23H2, macOS Sonoma 14.5)
*   **GUI 客户端类型**: (GUI.for.Clash 或 GUI.for.SingBox)
*   **客户端版本**: (例如: v1.9.0，可在“设置 -> 关于”页面查看)
*   **内核版本**:
*   **滚动发行版本**:
...
*   **复现步骤**: (你在问题出现前，具体执行了哪些操作？)

**3. 关键诊断文件**
*   **当前运行的完整配置文件**: (请导出并提供。**注意：在公开发布前，请务必删除或脱敏所有个人敏感信息，如服务器地址、密码、订阅链接等。**)
*   **错误日志与截图**: (请提供相关的**完整日志**、详细错误输出信息，或关键界面的**清晰截图/视频**。)`,
  },
];
