...
A: 请按以下顺序排查：
  ...
  1. **路径检查**：确保程序的可执行文件所在的完整路径中，不包含中文、空格或特殊字符。
...
`,
  ...
  {
    keywordGroups: [
      // 描述核心症状：开机/自启后，托盘图标变得透明/消失/无法点击
      [
        '开机|自启(动)?',
        '(托盘|图标)',
        '(变)?透明|消失|(空)?白|不见了|没了|(点)?不动|没反应|([不无没]法|不能)(点击|操作)',
      ],
      // 描述另一种常见情况：任务管理器里有进程，但托盘图标有问题
      ['(在|有)?.*?进程.*?(在|有)?', '托盘?图标', '(变)?透明|消失|不见了|点?不动'],
      // 英文匹配
      [
        '(on|after) (boot|startup|start)',
        'tray icon',
        'transparent|disappear|gone|missing|unclickable|no response|doesn.?t work|is invisible',
      ],
    ],
    answer: `
**Q: Windows 开机自启后，托盘图标透明、消失或无法点击？**
...
`,
  },
  {
    keywordGroups: [
      ...
      ['GUI|客户端|程序|软件|exe|安装包', '杀(了)?|报毒|报(是)?病毒|隔离|被删除|拦截|阻止|当(成|做)病毒'],
      ...
      // 模式四：描述安全软件的提示
      ['GUI|客户端|程序|软件', '报(有)?毒|是(不是)?病毒|有木马|提示.*?风险|提示.*?威胁|发现威胁'],
      // 英文模式
      [
        'GUI|client|program|exe|installer',
        'antivirus|defender|firewall|security',
        'delete|quarantine|block|remove|flagged as|report as',
      ],
...
**重要提示**: 添加排除后，如果文件已被隔离或删除，你需要**重新解压客户端**到该受信任的文件夹中，即可正常使用。
`,
  },
  {
    keywordGroups: [
      [
        ...
        '[没无]法|不能|失败|不动|卡住|报错|出问题|无效|没反应|没用|(还是|仍然|依然)老版',
      ],
      // --- 新增英文关键词 ---
...
`,
  ...
  {
    keywordGroups: [
      ['怎么|如何|咋|怎样', '更换|修改|换|改|自定义', '(托盘)?图标'],
...
`,
  },
  {
    ...
    answer: `
**Q: Linux 桌面系统上 GUI 文字位置偏高？**

A: 尝试安装 \`Noto-Sans-CJK\` 和 \`Microsoft-YaHei\` 字体后重启系统（此方法不一定适用于所有环境）。
`,
  },
  ...
  // 🌐 B.2.2 网络与订阅 (Network & Subscription)
  {
    keywordGroups: [
      ['403'],
      ['rate limit exceeded'],
      ['(github|api).*(限制|rate limit)'],
      ['超出', '速(率)?', '限制'],
      ['bad', 'credentials'],
    ],
    answer: `**
Q: GitHub API 速率限制 (403 rate limit exceeded)？**

  1. 访问你的 GitHub 开发者设置，生成一个新的 Personal Access Token (PAT)。
  2. 在软件的 **设置** -> **通用** 中，将获取的 Token 填入 **向 REST API 进行身份验证** 一栏。

Q: GitHub API 错误码 401 (bad credentials)？**

A: 请检查你 **向 REST API 进行身份验证** 中填写的 Token 是否正确，是否已过期。
`,
  },
  {
    keywordGroups: [
      // 场景一：订阅链接处理失败 (本次新增的核心)
      // 匹配：订阅链接无法识别/转换/解析/添加/导入/使用 等场景
      ['订阅(链接|地址)?', '([不没无]法?|不能|失败|出错|用不了|没反应)', '(识别|转换|解析|添加|导入)'],
      // 匹配更口语化的表达，如：“这个订阅怎么没反应”、“订阅链接不能自动识别”
      ['订阅(链接|地址)?', '(自动)?(识别|转换|解析)', '([不没无]法?|不能|失败|出错|不了)'],

      // 场景二：更新失败或无流量信息 (原场景优化)
      ...
      // 场景四：与其他客户端对比 (原场景强化)
      // 原规则过于严格，强化后可匹配更宽泛的对比抱怨
      [
        '为啥|为什么|怎么|咋',
        'verge|nekobox|clashx|其他客户端',
        '有|正常|显示|全',
        '(我|这里|这个|gui).*(不全|没有|少)',
      ],
      // 新增一个更灵活的规则，匹配 “nekobox能用，这里不行” 这种模式
      ['(nekobox|verge|clash)', '(可以|正常|能用|行)', 'gui.*([不没]行|[不没]能|[不没]法|用不了)'],

      // 场景五：英文关键词 (合并与优化)
      [
        'subscription',
        "no traffic (info)?|not a valid|fail(ed)? to update|doesn't work|can't (add|import|parse|recognize)",
      ],
      ['subscription', '(nodes?|proxies) (incomplete|missing)|not all nodes shown'],
      ['(it )?works in (verge|nekobox)', "but not here|doesn't work here"],
    ],
    excludeKeywords: [['tun'], ['网(络|站|页)?']],
    answer: `
**Q: 订阅更新失败、无流量信息或节点不完整/缺少？**
  ...
  // ⚙️ B.2.3 配置与导入 (Configuration & Import)
  {
    keywordGroups: [
      [
        ...
        '(自定义|自己|完整|(手)?(写|搓)(好)?|本地|订阅(提供|(下|分)发)|远程).*?(配置|文件|规则|策略|代理|分组)',
      ],
    ...
    answer: `
**Q: 如何导入/使用自己编写的完整配置文件？**

A: GUI.for.Cores 本身不直接支持导入完整的配置文件，这么设计是为了维持 GUI 操作的稳定性和一致性。但你可以通过以下特定功能，间接实现加载自定义配置的目的：

* **GUI.for.Clash**: 在添加订阅时，将你的完整配置文件托管在一个可访问的 URL 上（或存放在本地文件中），然后像添加普通订阅一样添加它，并启用 **使用订阅内的策略组和分流规则** 选项。这样，客户端会优先采用你文件中的 \`proxies\`, \`proxy-groups\`, 和 \`rules\` 部分。
...
`,
  ...
  {
    keywordGroups: [
      // 模式一：如何 + 动作 + 对象 (最常见，覆盖你的范例)
      [
        '(怎么|如何|咋|怎样)',
        '(快速|快捷|手动)?(导入|添加|加入|粘贴|使用)',
        '(单个|单独)?(的)?节点|分享链接|(ss|vmess|trojan)链接',
      ],
      // 模式二：特定协议的链接怎么用
      ['(ss|ssr|vmess|vless|trojan|hysteria2?|tuic|wireguard)(://)?', '(怎么|如何|咋|怎样)(导入|添加|使用)'],
      // 模式三：客户端 + 动作 + 对象 (覆盖 gfs 等简称)
      ['gfs|gui', '(怎么|如何|咋|怎样)(导入|添加|使用)', '(单个|单独)?(的)?节点|分享链接'],
      // 英文模式
      ['(how to|fastest way to)?', '(import|add|paste|use)', '(a )?(single|individual) node|(a )?share link'],
    ],
    answer: `
**Q: 如何快速导入单个节点分享链接 (如 ss://, vmess://)？**

A: 你可以通过【节点转换】插件，轻松地将单个节点链接转换为配置格式，并手动添加到客户端中。请按照以下步骤操作：
...
`,
  ...
  // 🕹️ B.2.3.1 活动连接规则 (Active Connection Rules)
  {
    keywordGroups: [
      // 场景一：直接提问“为什么不生效”
      ['连接|日志', '右键|添加', '直连|代理|拦截', '[不没无]生效|没(有)?(效果|反应)|不起作用|没用'],
      // 场景二：询问“如何使其生效”
      ['连接|日志', '(右键|添加)的?(规则)?', '怎么|如何|咋', '(让.*)?(生效|启用|起作用|应用)'],
      // 场景三：询问规则存储位置或如何编辑 (有上下文)
      ['连接|日志', '添加.*?(规则)?', '在(哪|哪里)|如何|怎么', '看|查看|找到|编辑|修改|删除|移除'],
      // 新增场景四：询问规则文件位置 (无上下文)
      ['添加(到)?', '直连|代理|拦截', '(哪个|什么|哪里).*(规则)?文件'],
      // 场景五：直接提到规则集文件名
      ['(direct|proxy|reject)(\\.(yaml|json))?', '怎么用|如何生效|不起作用'],
      // 场景六：更口语化的组合
      ['右键', '添加|设置', '直连|代理|拦截', '然后呢|下一步|怎么用'],
    ],
    answer: `
**Q: 在活动连接（日志）中右键添加的规则不生效？**
...
`,
  ...
  // 🐞 B.2.4 内核错误 (Core Errors)
  {
    ...
    excludeKeywords: [['(enabled|path|fakeip|rdrc)']],
    answer: `
**Q: 报错 "initialize cache-file: timeout"？**

A: sing-box 内核在启动时需要读写缓存文件（\`cache.db\`），此报错意味着该文件被另一个进程锁定或占用，这通常是由于旧的内核进程未能正常退出所致。
...
`,
  },
  {
    ...
    answer: `
**Q: 报错 "unknown field 'max_early_data'" 或相关类型错误？**

A: 部分订阅源提供的节点信息中，\`max_early_data\` 字段的值**不是规范的数字类型**，导致内核解析配置时因类型不匹配而失败。
...
`,
  ...
  // 🍏 B.2.6 macOS 专项 (macOS Specifics)
  {
    keywordGroups: [
      ...
      ['mac', '将对(你的)?电脑造成(伤害|损坏)|恶意软件'],

      // 场景二：通用描述无法打开的问题
      [
        'mac|苹果',
        '(软件|客户端|程序|app|应用)?',
        '(启动|运行)不(了|起来)|打不开|启动不了|([不没无]法|不能)(启动|运行|打开)|没反应|无响应|闪退|点不开|运行不了|一直(转圈)?加载',
      ],
      ...
      [
        'mac(系统|系统的)?',
        '(怎么|咋|为啥|就是)?(打不开|([不没无]法|不能)(启动|运行|打开)|启动不了|用不了|没反应)',
        '(求助|指导|怎么办|哪位|大(侠|佬|哥)?)?',
      ],
    ...
    answer: `
**Q: macOS 提示“已损坏”、“无法验证开发者”或“将对电脑造成伤害”，导致程序无法打开？**
...
\`\`\`
**重要提示**：在上面的命令最后（\`quarantine\` 后面）**需要加一个空格**，然后从 “访达” (Finder) 的 “应用程序” 文件夹中，**将无法打开的客户端程序图标拖拽到终端窗口中**，它会自动填充正确的路径，执行时会提示你输入电脑的开机密码（输入时密码不可见），输入后按回车即可。
...
* **macOS/Linux**: 前往 **设置 -> 内核** 页面，点击 **授予特权** 按钮为内核授权。
  * **如果需要为 TUN 网卡配置防火墙**: 请参考 Mihomo 的 [TUN 堆栈](https://wiki.metacubex.one/en/config/inbound/tun/#stack) 文档。
`,
  },
  {
    keywordGroups: [
      ['linux', '(授(予)?|给)?(特)?权(限)?', '[没无](反应|效(果)?|用)|点不了'],
...
A: Linux 上的授权操作依赖 \`pkexec\` 命令，请确保已安装提供此命令的软件包。
`,
  },
  {
    keywordGroups: [
    ...
    answer: `
**Q: 报错 "configure tun interface: The system cannot find the file specified."？**
  ...
  },
  {
    keywordGroups: [
      ...
      [
        ...
        '[没无]反应|^(?=.*([没无不]法|不能|连不上|访问不了|上不了|没))(?=.*网(络)?).*|断网|加载失败|无法加载|一直加载|不停转圈|请求超时',
      ],
      // --- 新增：精准匹配“打不开某个网站”的场景 ---
      [
        'tun',
        '(打不开|无法访问|访问不了|进不去|加载不了|显示不了)',
        '(某个|这个|特定|所有)?(网站|网页|链接|google|github|youtube|bilibili)',
      ],
      [
        'tun',
      ...
      [
        'tun',
        '(只能|仅|只有).*(tg|电报|telegram)',
        '(网页|网站|浏览器|其他|别的|剩下|剩余).*(打不开|没反应|用不了|加载不出来)',
      ],
      [
        'tun',
      ...
      [
        ...
        '(启动|运行|开)?',
        '内核|tun',
        '没网|断网|上不了网|^(?=.*(还|仍|依))(?=.*(系统|默认|自带))(?=.*DNS).*|^(?=.*([无没]法|不能|没有))(?=.*(接管|劫持|生效|用))(?=.*DNS).*',
      ],
      ...
      [
        'tun',
        '系统代理',
        '才|必须|要开|依赖|同时|可以|访问不了', // 描述 TUN 和系统代理的依赖关系问题
      ],
      ...
      // 模式二：因果关系
      [
        '(when|after) (i )?(enable|turn on) tun',
        '(i )?(lose|lost) internet|no network|sites won.?t load|images fail to load',
      ],
    ...
    answer: `
**Q: TUN 模式启动后无法上网或网络异常？只启用 TUN 无法访问网站？必须同时开启系统代理才能访问网站？**

A: 请按以下顺序排查，方案覆盖 Windows, macOS 及 Linux：
* **更换 TUN 堆栈**
  * 在配置设置中尝试更换 **TUN 堆栈** 为 **GVisor**。

* **修改系统 DNS**
  * **原因**: TUN 在 macOS 上无法劫持发往局域网的 DNS 请求。
  * **解决方案**: 将你 Mac 的系统 DNS 修改为任意公共 DNS 服务器（例如 \`8.8.8.8\`）。

* **检查防火墙**
  * 检查 Windows 防火墙设置，确保内核程序未被阻止。
  * Linux 参考 Mihomo 的 [TUN 堆栈](https://wiki.metacubex.one/en/config/inbound/tun/#stack) 文档
...
* **修改 IP 地址前缀**
  * 前往 **配置设置 -> 入站设置** -> \`tun-in\`，尝试修改 **IP 地址前缀** 为一个冷门的私有网段，避免与当前局域网或其他网络接口产生冲突。
`,
  },
