...
2. 在客户端的 **设置 -> 通用** 中，将获取的 Token 填入 **向 REST API 进行身份验证** 一栏。`,
  },
  {
    keywordGroups: [
      ...
      ['为啥|为什么|怎么|咋', 'verge|nekobox|clashx|其他客户端', '有|正常|显示|全', '(我|这里|这个|gui).*(不全|没有|少)'],
      // 新增一个更灵活的规则，匹配 “nekobox能用，这里不行” 这种模式
      ['(nekobox|verge|clash)', '(可以|正常|能用|行)', 'gui.*([不没]行|[不没]能|[不没]法|用不了)'],

      // 场景五：英文关键词 (合并与优化)
      ['subscription', "no traffic (info)?|not a valid|fail(ed)? to update|doesn't work|can't (add|import|parse|recognize)"],
