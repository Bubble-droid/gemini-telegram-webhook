...
import type { OpenAiAgent } from '@llm/agent/openai-agent';
import { convertToOpenAiContent } from '@llm/lib/converter';
...
import { hasImage } from '@shared/utils/message';
...
export class ChitchatHandler {
  ...
  constructor(agent: OpenAiAgent, fileHandler: FileHandler) {
    this.agent = agent;
   ...
   * 现在的入口方法，确保只处理一次
   */
  public handle(ctx: ResponseContext, messages: Message[]): Promise<boolean> {
    return this.runSequential(ctx.chat.id, () => {
      return this.handleMessage(ctx, messages);
    });
  }
   ...
   */
  private async handleMessage(ctx: ResponseContext, messages: Message[]): Promise<boolean> {
    const { chat } = ctx;
    ...
    const imageParts = await this.fileHandler.batchProcessFiles(messages, hasImage);

    const rawTexts = messages.map((m) => m.text ?? m.caption ?? '').filter((t) => t.length > 0);

    if (imageParts.length === 0 && rawTexts.length === 0) {
      logger.trace(`[ChitChat] No message content.`, { chatId: chat.id });
      return false;
    }

    messageParts.push(...imageParts.map(convertToOpenAiContent));

    const contextMarkdown = messages.map(formatContextToMarkdown).join('\n\n---\n\n');

    messageParts.push({ type: 'text', text: contextMarkdown });
    ...
    // 3. 计算注意力分
    const weight = messages.map((m) => this.calculateMessageWeight(m)).reduce((a, b) => a + b, 0);
  ...
  private async requestChat(state: ChitchatState, ctx: ResponseContext): Promise<string | null> {
    ...
    try {
      const completion = await this.agent.run(messages);

      this.appendMessage(state, completion);
