import { DATA_DIR, FILE_ID_FILE, GITHUB_BASE_URL } from '@shared/core/constants';
import { logger } from '@shared/core/logger';
import type { Recordable } from '@shared/types/common';
import { invertObject } from '@shared/utils/helpers';
...
const FILE_ID_PATH = path.join(DATA_DIR, FILE_ID_FILE);

const loadFileIdMap = async (): Promise<Map<string, string>> => {
  const fileIds = await loadData<Recordable<string>>(FILE_ID_PATH, 'json');
...
class PathResolver {
  private fileIdMap: Map<string, string> | undefined;

  public resolve(rawId: string): string {
    const cleanId = rawId.split('-0-0-')[1];
    if (!cleanId) return 'N/A';
    logger.debug(`Resolving path for ID: ${cleanId}`);

    const rawPath = this.fileIdMap?.get(cleanId);
  ...
  public async loadFileIdMap() {
    this.fileIdMap = await loadFileIdMap();
  }
}
